...
!(async () => {
  ...
  let lnStartAcc=Math.ceil(cookiesArr.length/10)*6;
  let lnTotalAcc=Math.ceil(cookiesArr.length/10)*7;
  
  if (lnTotalAcc>cookiesArr.length){
	  lnTotalAcc=cookiesArr.length;
  }
  if (lnStartAcc>=lnTotalAcc){
	  console.log(`账号太少不需要第七个任务,跳出\n`);
	  return 
  }
...
async function signInit() {
  return new Promise(resolve => {
    $.get(taskUrl('speedSignInit', {
      ...
      "kernelPlatform": "RN",
      "inviterId":"Oth5XdvSwVHdwAC2UslVFO/iDeDmDQspVnKodJhgQPg="
    }), async (err, resp, data) => {
      try {
        if (err) {
